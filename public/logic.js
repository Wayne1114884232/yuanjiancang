...
function latestTierPrice(state,partId,qty){const prices=(state.observations||[]).filter(o=>o.partId===partId&&o.source==='立创商城'&&Number.isFinite(o.price)).sort((a,b)=>Date.parse(b.asOf)-Date.parse(a.asOf));const newest=prices[0]?.asOf;if(!newest)return null;const tiers=prices.filter(o=>o.asOf===newest).map(o=>({...o,tier:Number(String(o.quantityTier).match(/\d+/)?.[0]||1)})).filter(o=>o.tier<=qty).sort((a,b)=>b.tier-a.tier);return tiers[0]||prices.find(o=>o.asOf===newest)||null;}
...
export function collectAlerts(state) {
  ...
  for(const o of state.observations){if(!state.parts.some(p=>p.id===o.partId&&p.watch))continue;const key=JSON.stringify([o.partId,o.source,o.currency,o.quantityTier]);if(!groups.has(key))groups.set(key,[]);groups.get(key).push(o);}
