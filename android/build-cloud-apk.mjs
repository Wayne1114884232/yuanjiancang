...
let manifest=fs.readFileSync(dst+'/app/src/main/AndroidManifest.xml','utf8').replace('android:allowBackup="true"','android:allowBackup="false"').replace('<uses-permission android:name="android.permission.INTERNET" />','<uses-permission android:name="android.permission.INTERNET" /><uses-permission android:name="android.permission.REQUEST_INSTALL_PACKAGES" />').replace('</application>','<provider android:name=".UpdateFileProvider" android:authorities="com.wayne.componenthub.team.updates" android:exported="false" android:grantUriPermissions="true" /></application>').replace(/\s*<intent-filter android:autoVerify="true">[\s\S]*?<\/intent-filter>/,'');fs.writeFileSync(dst+'/app/src/main/AndroidManifest.xml',manifest);
