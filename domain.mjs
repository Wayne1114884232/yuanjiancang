...
export function emptyState() { return {schemaVersion:1,rev:0,createdAt:now(),locations:['公司','学校','家'].map(name=>({id:id(),name,note:''})),parts:[],stocks:[],lots:[],lotMovements:[],orders:[],events:[],observations:[],projects:[],reservations:[],procurementItems:[],usageRecords:[],substitutions:[],containers:[],finishedGoods:[],finishedGoodLogs:[],settings:{priceThreshold:10,lcscEnabled:true,lcscLastSync:null,lcscLastSyncError:'',feedUrl:'',feedEnabled:false,lastSync:null,lastSyncError:''}}; }
...
export function normalizePart(raw, existing = {}) {
  ...
  p.minStock=integer(raw.minStock??0,'安全库存'); p.watch=Boolean(raw.watch); p.critical=Boolean(raw.critical);
...
function postOrder(s, {type, note='', boards=1, source='手动', lines, requestId, projectId='', procurementItemId='',lotCode='',supplier='',labelText='',unitPrice=null,purchaseDate='',reverseOrderId='',receivedLots=[],time:requestedTime=''}) {
  ...
  unitPrice=unitPrice===''||unitPrice==null?null:decimal(unitPrice,'采购单价');purchaseDate=validDate(purchaseDate,'采购日期');
  const recordedAt=now(),orderTime=requestedTime?receiptTime(requestedTime,recordedAt):recordedAt;
  const order={id:id(),time:orderTime,recordedAt,type,unitPrice,purchaseDate,note:clean(note,2000),lotCode:clean(lotCode,300),supplier:clean(supplier,300),labelText:clean(labelText,6000),boards,source:clean(source),lines:copy(lines),requestId:clean(requestId),projectId:clean(projectId),procurementItemId:clean(procurementItemId),undoneBy:null};
  recordLots(s,order,{reverseOrderId,receivedLots});
  for (const [stockId,delta] of aggregate) {
    if (!delta) continue;
    const stock=s.stocks.find(x=>x.id===stockId); const before=stock.qty; stock.qty+=delta;
    const used=s.lotMovements.filter(x=>x.stockId===stock.id&&x.orderId===order.id).map(x=>({...x,lotCode:s.lots.find(l=>l.id===x.lotId)?.lotCode}));
    s.events.push({id:id(),orderId:order.id,time:order.time,recordedAt,stockId,partId:stock.partId,locationId:stock.locationId,bin:stock.bin,before,delta,after:stock.qty,type,note:order.note,lotCode:used.map(x=>x.lotCode).filter(Boolean).join(',')||order.lotCode,supplier:order.supplier,lotUsage:used.map(x=>({lotId:x.lotId,lotCode:x.lotCode,delta:x.delta}))});
  }
...
export function applyAction(original, action) {
  ...
  if(action.type==='label.receive') {
    ...
    result.order=postOrder(s,{type:'入库',source:'标签核对入库',note:action.note,lotCode:action.lotCode,supplier:action.supplier,unitPrice:action.unitPrice,purchaseDate:action.purchaseDate,labelText:action.labelText,time:action.time,lines:[{stockId:stock.id,delta:Number(action.qty),base:Number(action.qty),loss:0}],requestId:action.requestId});result.partId=partId;
    ...
    for(const row of action.rows){
      ...
      const out=applyAction(next,{type:'stock.post',stockId:row.stockId,kind:row.kind,qty:row.qty,boards:row.boards??1,loss:row.loss??0,note:row.note||action.note,time:row.time||action.time,lotId:row.lotId,lotCode:row.lotCode,supplier:row.supplier,unitPrice:row.unitPrice,purchaseDate:row.purchaseDate,requestId:action.requestId});next=out.state;orders.push(out.order);
    }
    ...
    if (!existing && !action.skipStock) {
      ...
      if (qty) postOrder(s,{type:'入库',source:'新建元件',note:'初始入库',lotCode:action.lotCode,supplier:action.supplier,unitPrice:action.unitPrice,purchaseDate:action.purchaseDate,time:action.time,lines:[{stockId:stock.id,delta:qty,base:qty,loss:0}],requestId:action.requestId});
    } result.partId=p.id;
    ...
    result.order=postOrder(s,{type:action.kind,note:action.note,boards,time:action.time,lotCode:action.lotCode,supplier:action.supplier,unitPrice:action.unitPrice,purchaseDate:action.purchaseDate,receivedLots:action.receivedLots,lines:[{stockId:stock.id,delta,base,loss,lotId:action.lotId,lotCode:action.lotCode}],requestId:action.requestId});
    ...
    const lines=action.rows.map(row=>{const stock=s.stocks.find(x=>x.id===row.stockId);assert(stock,'批量出库包含不存在的库位');const qty=integer(row.qty,'出库数量',1);assert(stock.qty>=qty,`${s.parts.find(p=>p.id===stock.partId)?.sku||'元件'} 库存不足：需 ${qty}，现有 ${stock.qty}，整批未提交`);return {stockId:stock.id,delta:-qty,base:qty,loss:0,lotId:row.lotId};});
    ...
    const lines=includedRows.map(row=> {
      ...
      return {stockId:stock.id,delta:action.kind==='入库'?base:-(base+loss),perBoard,base,loss,lotId:row.lotId,unitPrice:row.unitPrice,reference:clean(row.reference,500)};
    });
    result.order=postOrder(s,{type:action.kind,note:action.note,boards,time:action.time,source:'BOM',lotCode:action.lotCode,supplier:action.supplier,lines,requestId:action.requestId});
  ...
  } else if(action.type==='project.pickSave'){
    result.pickSession=savePicking(s,action);
    ...
    for(const row of project.rows){const base=row.perBoard*boards,loss=Math.ceil(base*rate/100)+row.loss,qty=base+loss;assert(partTotal(s,row.partId)-reservedTotal(s,row.partId,project.id)>=qty,`${s.parts.find(x=>x.id===row.partId)?.sku} 可用库存不足`);const pick=action.pickSessionId?checkedPicking(s,project,action):null;assert(!project.pickSession||project.pickSession.status!=='picking'||pick,'请从已保存的拣料单确认出库');const allocated=pick?pick.rows.filter(x=>x.partId===row.partId).map(x=>({stockId:x.stockId,delta:-x.qty,base:x.qty,loss:0,lotId:x.lotId})):allocatePart(s,row.partId,qty,action.locationId);let remainingLoss=loss;for(const line of allocated){line.loss=Math.min(remainingLoss,-line.delta);line.base=-line.delta-line.loss;remainingLoss-=line.loss;}all.push(...allocated);}
    ...
    const item=s.procurementItems.find(x=>x.id===action.itemId);assert(item&&item.status!=='cancelled','采购项不存在或已取消');const qty=integer(action.qty,'到货数量',1);assert(item.orderedQty>0&&item.receivedQty+qty<=item.orderedQty,`本次最多可登记 ${Math.max(0,item.orderedQty-item.receivedQty)} 个`);const stock=findStock(s,item.partId,action.locationId,action.bin||'');result.order=postOrder(s,{type:'入库',source:'采购到货',note:action.note||item.note||item.supplier,time:action.time,lotCode:action.lotCode,supplier:item.supplier,unitPrice:action.actualUnitPrice??item.actualUnitPrice,lines:[{stockId:stock.id,delta:qty,base:qty,loss:0}],requestId:action.requestId,procurementItemId:item.id});item.receivedQty+=qty;item.status=item.receivedQty>=item.orderedQty?'received':'partial';item.updatedAt=now();
    ...
    assert(s.parts.some(x=>x.id===action.partId),'元件不存在');if(action.projectId)assert(s.projects.some(x=>x.id===action.projectId),'项目不存在');const record={id:id(),partId:action.partId,lotId:clean(action.lotId),testedAt:action.testedAt?receiptTime(action.testedAt,now()):now(),projectId:clean(action.projectId),condition:clean(action.condition,1500),result:clean(action.result,1500),issue:clean(action.issue,1500),createdAt:now()};if(record.lotId)assert(s.lots.some(l=>l.id===record.lotId&&s.stocks.some(st=>st.id===l.stockId&&st.partId===record.partId)),'实测批次不属于此元件');assert(record.condition||record.result||record.issue,'请填写测试条件、结果或问题');s.usageRecords.push(record);result.usageRecordId=record.id;
  } else if(action.type==='substitution.save'){
    assert(s.parts.some(x=>x.id===action.partId)&&s.parts.some(x=>x.id===action.substitutePartId)&&action.partId!==action.substitutePartId,'请选择两个不同的有效元件');if(action.projectId)assert(s.projects.some(x=>x.id===action.projectId),'项目不存在');
    const existing=action.id?s.substitutions.find(x=>x.id===action.id):null;assert(!action.id||existing,'替代关系不存在');assert(!existing||existing.partId===action.partId&&existing.substitutePartId===action.substitutePartId,'不能修改替代料身份');
    const status=['candidate','verified','rejected'].includes(action.status)?action.status:'candidate';
    const sub={...existing,id:existing?.id||id(),partId:action.partId,substitutePartId:action.substitutePartId,status,projectId:clean(action.projectId),conditions:clean(action.conditions,1500),note:clean(action.note,1500),createdAt:existing?.createdAt||now(),updatedAt:now(),history:[...(existing?.history||[]),{status,conditions:clean(action.conditions,1500),note:clean(action.note,1500),at:now()}]};
    assert(status!=='verified'||sub.conditions,'已验证替代料必须填写验证条件');assert(status!=='rejected'||sub.note,'判定不适用请填写原因');
    assert(existing||!s.substitutions.some(x=>x.partId===sub.partId&&x.substitutePartId===sub.substitutePartId&&x.projectId===sub.projectId),'已存在相同替代关系，请更新原记录');
    if(existing)s.substitutions[s.substitutions.indexOf(existing)]=sub;else s.substitutions.push(sub);result.substitutionId=sub.id;
    ...
    const reversal=postOrder(s,{type:'撤销',reverseOrderId:order.id,source:'撤销记录',note:`撤销 ${order.source} ${order.type}：${clean(action.reason||action.note)||order.note}`,lines:affected.map(e=>({stockId:e.stockId,delta:-e.delta})),requestId:action.requestId}); order.undoneBy=reversal.id; reversal.undoOf=order.id; result.order=reversal;
...
export function validateBackup(raw) {
  ...
  assert(s.settings&&typeof s.settings==='object','备份设置无效'); s.settings.lcscEnabled=s.settings.lcscEnabled!==false;s.settings.lcscLastSync=s.settings.lcscLastSync||null;s.settings.lcscLastSyncError=clean(s.settings.lcscLastSyncError,500);s.settings.feedUrl=url(s.settings.feedUrl); assert(!s.settings.feedUrl||s.settings.feedUrl.startsWith('https://'),'数据源须使用HTTPS'); decimal(s.settings.priceThreshold,'阈值',0.1,1000); validateLots(s); return s;
}
