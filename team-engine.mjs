import {migrateLots} from './lots.mjs';
...
const allowedActions=new Set(['label.receive','part.save','parts.bulkUpdate','location.save','settings.save','stock.post','stock.batch','stock.bulkPost','bom.post','project.save','project.reserve','project.release','project.issue','project.pickSave','procurement.bulkAdd','procurement.save','procurement.receive','procurement.cancel','usage.record','substitution.save','container.save','container.assign','container.stocktake','finishedGood.post','stock.transfer','order.undo','price.record','library.restore']);
...
export class TeamEngine {
  ...
  viewState(db,l,m){
    const s=structuredClone(l.state);migrateLots(s);const loc=id=>this.locationAllowed(m,id),proj=id=>this.visibleProject(m,id);
    ...
    const orders=new Set(s.orders.map(x=>x.id));s.lots=s.lots.filter(x=>stockIds.has(x.stockId));s.lotMovements=s.lotMovements.filter(x=>stockIds.has(x.stockId)&&orders.has(x.orderId));for(const lot of s.lots)if(!orders.has(lot.receiptOrderId))delete lot.receiptOrderId;s.events=s.events.filter(x=>stockIds.has(x.stockId)&&orders.has(x.orderId));
    s.projects=s.projects.filter(x=>proj(x.id));for(const p of s.projects)if(p.pickSession)p.pickSession.rows=p.pickSession.rows.filter(x=>stockIds.has(x.stockId));s.reservations=s.reservations.filter(x=>proj(x.projectId));s.procurementItems=s.procurementItems.filter(x=>proj(x.projectId));s.usageRecords=s.usageRecords.filter(x=>proj(x.projectId));s.substitutions=s.substitutions.filter(x=>proj(x.projectId));
  ...
  authorize(db,user,l,m,a){
    ...
    if(a.type==='part.save'&&a.part?.critical)check(manage(m),'标记关键元件需要管理员权限',403);
    if(a.type==='part.save'&&a.part?.id)check(manage(m),'编辑共享元件资料需要管理员权限',403);
    if(a.type==='order.undo')check(text(a.reason||a.note,500),'请填写撤销原因');
    if(a.type==='usage.record'&&a.lotId){const lot=safeLot(l.state,a.lotId);check(this.locationAllowed(m,l.state.stocks.find(st=>st.id===lot.stockId)?.locationId),'没有此批次地点的权限',403);}
  ...
  prepare(db,user,l,m,a){this.authorize(db,user,l,m,a);let result;
    ...
    const reduced=new Set(next.events.slice(before.events.length).filter(e=>e.delta<0).map(e=>e.partId));const critical=next.parts.filter(p=>p.critical&&reduced.has(p.id));check(!critical.length||a.confirmCritical===true,'本次涉及关键元件，请核对后勾选关键元件确认');
    for(const x of next.usageRecords.slice(before.usageRecords.length)){x.actorId=user.id;x.actorName=user.username;}
    if(a.type==='substitution.save'){const sub=next.substitutions.find(x=>x.id===result.substitutionId);sub.actorName=user.username;sub.history.at(-1).actorName=user.username;}
    if(a.type==='project.pickSave'){const p=next.projects.find(x=>x.id===a.projectId);p.pickSession.ownerName=user.username;p.pickSession.ownerId=user.id;}
  ...
  commitAction(db,user,l,a,out){const before=l.state;const changes=[];
    for(const st of out.state.stocks){const old=before.stocks.find(x=>x.id===st.id);if((old?.qty||0)!==st.qty)changes.push({kind:'库存',label:out.state.parts.find(x=>x.id===st.partId)?.sku||st.partId,locationId:st.locationId,bin:st.bin,before:old?.qty||0,after:st.qty});}
    for(const p of out.state.parts){const old=before.parts.find(x=>x.id===p.id);for(const k of ['sku','name','category','package','value','voltage','tolerance','manufacturer','minStock','critical'])if(!same(old?.[k],p[k]))changes.push({kind:k,label:p.sku,before:old?.[k]??'',after:p[k]??''});}
    if(a.type==='library.restore')this.backupBefore(l,'恢复前自动备份');l.state=out.state;this.audit(db,l.id,user,a.type,{rev:l.state.rev,orderId:out.order?.id||'',kind:a.kind||'',reason:text(a.reason||a.note,500),changes:changes.slice(0,300),changesCount:changes.length});}
  ...
  transfer(key,fromId,a){return this.transaction(db=>{const {user}=this.auth(db,key),src=this.access(db,user.id,fromId),dest=this.access(db,user.id,a.toLibraryId);check(src.library.id!==dest.library.id,'请选择另一个元件库');check(manage(src.member)&&manage(dest.member),'跨库调拨需要你在两端都有管理员权限',403);check(!src.library.policy.approvals&&!dest.library.policy.approvals,'启用敏感操作审批的库暂不支持跨库调拨，请由所有者关闭规则后操作');const prior=this.receipt(db,user,fromId,{...a,type:'library.transfer'});if(prior)return {duplicate:true,...prior.result};check(a.rev===src.library.state.rev&&a.toRev===dest.library.state.rev,'两端库存已变化，请重新核对调拨',409);const st=src.library.state.stocks.find(x=>x.id===a.stockId);check(st,'来源库位不存在');check(int(a.qty,1,1e9),'数量须为正整数');const original=src.library.state.parts.find(x=>x.id===st.partId),tid=uid(),note=`跨库调拨 ${tid} ${text(a.note,500)}`;
    const out=this.prepare(db,user,src.library,src.member,{type:'stock.post',stockId:st.id,kind:'出库',qty:Number(a.qty),lotId:a.lotId,confirmCritical:a.confirmCritical,note,requestId:a.requestId});let targetState=structuredClone(dest.library.state),p=targetState.parts.find(x=>x.sku.toLowerCase()===original.sku.toLowerCase());
...
function safeLot(s,id){const lot=s.lots?.find(x=>x.id===id);check(lot,'批次不存在');return lot;}
